...
angular.module('yarneeApp.controllers').controller('create-yarnCtrl',  function($scope, $rootScope, $routeParams, $window, $http, $location, $timeout) {
  ...
  $scope.addNewPageOnStartup  = ($routeParams.newpage || 'NO').replace(/[_/-]+/g, '') == 'newpage';
  $scope.startOnPage          = !$scope.addNewPageOnStartup && parseInt(($routeParams.newpage || 'NO').replace(/\//g, '')) > 0 ? parseInt($routeParams.newpage.replace(/\//g, '')) : false;
  ...
  $scope.uploadFiles = function(url, files) {
    ...
    var s3upload = s3upload != null ? s3upload : new S3Upload({
      ...
      onFinishS3Put: function(public_url) { // Get the URL of the uploaded file
        ...
        var titleActive = $('.tile.active');

        titleActive.css('background',      'url("'+$scope.page.content[0].meta.previewimage+'") no-repeat center center');
        titleActive.css('background-size', 'cover'); // TODO currently does not work when setting this in pure CSS
  ...
  $scope.selectressource = function($event) {
    ...
    titleActive.css('background',      'url("'+$scope.page.meta.previewimage+'") no-repeat center center');
    titleActive.css('background-size', 'cover');
